...
const isValidEmail = (email) => /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email); // checks if mail has @ or not
...
const Signup = ({ onSignupSuccess = null }) => {
  ...
  const handleSubmit = async (ev) => {
    ...
    try {
      ...
      const respo = await fetch(`${API_BASE}/api/auth/register`, {
        method: "POST",
            ...
            <div className={signupStyles.formContent}>
              <h2 className={signupStyles.heading}>
                <span className={signupStyles.headingIcon}>
                  <CheckCircle className={signupStyles.headingIcon} />
                </span>
